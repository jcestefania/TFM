...
#goal[:] = 0 # Comentar esta línea para fijar el objetivo (uno ya definido previamente, que está en el JSON, en matriz_indicios_uni.py), si no, coge uno aleatorio. La posición (0,0) la defino como el caso en que se omite lo que está en el JSON (generar uno aleatorio en objetivo_a.py) 
# Comentado -> posición fija
# Descomentado -> aleatorio con semilla
...
print(df)
guardar_dataframe(
    df, "./resultados/", "ldfs-heur", file.split("/")[-1].split(".")[0]
)
...
# Generar archvo .plan
if GENERAR_PLAN:
    latitude_0, longitude_0 = reference_coordinate #mis coordenadas del centro del espacio de búsqueda
