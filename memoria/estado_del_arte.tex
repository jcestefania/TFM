...
% DISEÑO DE LOS TÍTULOS de las partes del trabajo
\usepackage{titlesec}
\usepackage{titletoc}
...
%BIBLIOGRAFÍA 
\usepackage[backend=bibtex, style=ieee, isbn=false,sortcites, maxbibnames=6, minbibnames=1]{biblatex}
...
\begin{titlepage}
	...
	\begin{center}
		...
		\end{Large}
		 	{\Huge Estado del Arte}\\
		 	\vspace*{0.3cm}
		 	{\Large Enrutamiento inteligente sobre mapas vectorizados para el despliegue de drones en búsqueda y rescate.}\\
		 	\vspace*{0.5cm}
	 		\rule{10.5cm}{0.1mm}\\
			\vspace*{0.9cm}
...
Los vehículos aéreos no tripulados (UAVs o drones) han revolucionado las operaciones de búsqueda y rescate (SAR) en entornos salvajes, proporcionando reconocimiento aéreo que expande tanto el campo de visión como la velocidad operativa de los equipos humanos. Según Queralta et al.\cite{queralta2020}, los sistemas multi-robot colaborativos para SAR requieren planificación, coordinación, percepción y visión activa para maximizar la eficiencia de las misiones.

La planificación de rutas para drones en SAR presenta desafíos únicos: entornos vastos y complejos, tiempo crítico donde la probabilidad de supervivencia disminuye rápidamente\cite{koester2008}, y la necesidad de coordinar múltiples agentes de forma eficiente. Este estado del arte revisa las técnicas de búsqueda, algoritmos de planificación de rutas y métricas de evaluación relevantes para el despliegue de drones en operaciones SAR.

El trabajo se estructura en torno a cuatro pilares fundamentales: (1) técnicas de planificación de rutas para UAVs, (2) algoritmos de búsqueda informada y no informada, (3) métricas de evaluación de rendimiento, y (4) modelos de comportamiento de personas perdidas basados en datos empíricos.

\chapter{Planificación de Rutas para UAVs en SAR}
...
\section{Coverage Path Planning (CPP)}
...
\section{Algoritmos Baseline en SAREnv}

El framework SAREnv\cite{grontved2025} proporciona cuatro algoritmos baseline para la evaluación comparativa de estrategias de búsqueda con drones:
...
\section{Métodos Metaheurísticos}
...
\section{POMDP y Procesos de Decisión}
...
\section{Probabilidad de Detección Acumulada}
...
\section{Probabilidad de Detección con Descuento Temporal}
...
El factor de descuento $\lambda$ se calcula basado en un tiempo terminal $t_{end}$ por el cual se desea acumular el 95\% del valor total descontado. Este cálculo es especialmente importante en dominios como operaciones SAR donde la certeza de la ubicación de una persona perdida exhibe decaimiento temporal.

\section{Puntaje de Descubrimiento de Personas Perdidas}
...
\section{Probabilidad de Éxito (POS)}
...
\chapter{Modelos de Comportamiento de Personas Perdidas}
...
\section{ISRID y Lost Person Behavior}
...
\section{Generación de Mapas de Probabilidad}

SAREnv\cite{grontved2025} utiliza un modelo probabilístico basado en estadísticas de comportamiento de personas perdidas. El proceso comienza extrayendo características geoespaciales de OpenStreetMap (estructuras, caminos, drenajes, agua, vegetación) y asigna probabilidades según tablas estadísticas derivadas de datos empíricos.
...
\chapter{Mapas Vectorizados y Datos Geoespaciales}
...
\section{Sistemas de Información Geográfica (GIS)}
...
\chapter{Conclusiones del Estado del Arte}
