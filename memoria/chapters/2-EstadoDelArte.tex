...
\label{sec:teoria_busqueda_bayesiana}

La base matemática para la planificación de misiones de búsqueda tiene sus raíces teóricas en los trabajos desarrollados durante la Segunda Guerra Mundial, documentados por autores seminales como Koopman \cite{koopman1980}. El problema fundamental no consiste únicamente en mover un dron por el espacio, sino en decidir cómo moverlo para maximizar las posibilidades de encontrar el objetivo antes de que se agote el tiempo o la batería.
...
\begin{itemize}
    ...
    \begin{equation}
        POS = POA \times POD
    \end{equation}
    Cabe señalar que en la doctrina de búsqueda y salvamento (como el manual IAMSAR), la Probabilidad de Área ($POA$) se denomina indistintamente Probabilidad de Contención ($POC$), siendo formulaciones conceptual y matemáticamente equivalentes ($POS = POA \times POD \equiv POC \times POD$). Un algoritmo inteligente puede decidir buscar en una zona con menos probabilidad de presencia (baja POA/POC) si el terreno está muy despejado y es fácil ver el suelo (alta POD), garantizando así un mayor éxito global.
\end{itemize}
...
Según el manual IAMSAR, la Probabilidad de Éxito (POS) es el producto de la Probabilidad de Contención (POC, conceptualmente equivalente a la POA definida en la Sección~\ref{sec:teoria_busqueda_bayesiana}) y la Probabilidad de Detección (POD):
...
\begin{table}[H]
	\ttabbox[\textwidth]
	{\caption{PARÁMETROS ESTADÍSTICOS DE DISPERSIÓN RADIAL Y DESPLAZAMIENTO SEGÚN ISRID \protect\cite{koester2008lost}}}
...
\subsection{Generación de Mapas de Probabilidad en la Literatura}

La traslación de las estadísticas de comportamiento humano hacia modelos computacionales ejecutables en robótica aérea fue formulada recientemente por Grøntved et al. a través del entorno SAREnv \cite{drones9090628}. El paradigma propuesto articula la construcción del mapa de creencias a partir de la descomposición semántica de características geoespaciales vectoriales obtenidas de OpenStreetMap (estructuras edificadas, vías de comunicación, redes de drenaje e hidrología, y coberturas vegetales). 

A cada entidad geográfica se le asocian probabilidades empíricas condicionadas por el perfil psicológico y demográfico del sujeto mediante dos factores complementarios:
\begin{enumerate}
	\item \textbf{Atracción tipológica:} Determinada por la frecuencia histórica de hallazgo asociada al tipo de terreno o infraestructura según los catálogos LPB e ISRID \cite{koester2008lost, romero2018manual}.
	\item \textbf{Decaimiento radial:} Modelado a partir del LKP mediante distribuciones continuas de distancia. Los estudios empíricos demuestran que la distribución log-normal proporciona el mejor ajuste a los datos de dispersión ($\text{RMSE} = 1{,}76\text{ km}$), superando significativamente a las funciones gaussiana normal ($\text{RMSE} = 6{,}83\text{ km}$) y gamma ($\text{RMSE} = 2{,}02\text{ km}$).
\end{enumerate}

La integración de ambos factores genera una puntuación combinada que, tras ser agregada y normalizada sobre el espacio de búsqueda, produce un mapa de Probabilidad de Área ($POA$) estadísticamente consistente que satisface $\sum POA = 1{,}0$ \cite{doherty2014}. 

La formulación matemática rigurosa de este proceso, así como su adaptación metodológica mediante agregación lineal ponderada multicapa, discretización espacial en celdas métricas de $10\text{ m}$ y filtrado jerárquico de no transitabilidad sobre el escenario real de la Casa de Campo de Madrid, se desarrollan en profundidad en el Capítulo \ref{cap:modelado}.
