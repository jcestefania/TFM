...
\begin{figure}[H]
	...
	\begin{tikzpicture}[node distance=1.5cm, auto,
	    block/.style={rectangle, draw=azulUC3M, fill=gray97, text width=2.7cm, text centered, rounded corners, minimum height=1.05cm, font=\footnotesize},
	    line/.style={draw=azulUC3M, thick, -latex}]
	    
	    \node [block] (in) {Matriz Base\\$\mathbf{POA}_{\text{base}}$};
	    \node [block, below of=in, node distance=2.2cm] (polys) {Polígonos Exclusión\\(Agua y Edificios)};
	    \node [block, right of=in, xshift=2.2cm, yshift=-1.1cm, text width=2.9cm] (mask) {\textbf{Máscara R-Tree}\\Anulación Estricta:\\$P(i,j) = 0{,}0$};
	    \node [block, right of=mask, node distance=3.6cm, text width=2.9cm] (renorm) {\textbf{Renormalización}\\Global:\\$\sum_{i,j} b(v^0) = 1{,}0$};
	    \node [block, right of=renorm, node distance=3.4cm, text width=2.5cm] (out2) {Mapa de Creencias\\$b(v^0)$};
...
\begin{itemize}
    \item \texttt{size}: Dimensiones matriciales de la rejilla $[N_y, N_x] = [458, 481]$ celdas para la Casa de Campo.
...
\begin{enumerate}
    \item \textbf{Tasa de Éxito de Misión ($SR_{\text{misión}}$, \%):} Porcentaje de misiones independientes en las que el dron intercepta la víctima virtual a una distancia $d \le 50\text{ m}$. Asimismo, el evaluador calcula la tasa de cobertura sobre el censo de víctimas virtuales ($SR_{\text{pob}}$).
    \item \textbf{Pasos hasta el Hallazgo (\textit{Steps to Goal}):} Número de instantes temporales transcurridos hasta la detección de la víctima.
    \item \textbf{Puntuación de Verosimilitud Acumulada ($S_{\text{like}}$, \%):} Suma acumulada de la creencia a priori $b(v^0)$ en las celdas sobrevoladas por el sensor ($\sum_{(i,j) \in \mathcal{V}} b(v^0)(i,j)$), métrica analítica de absorción de probabilidad que coincide con la tasa esperada de rescate poblacional ($S_{\text{like}} \approx SR_{\text{pob}}$).
    \item \textbf{Área Geográfica Barrida (\textit{Covered Area}, $\text{km}^2$):} Superficie neta cubierta por la huella del sensor sin duplicar solapamientos ($A_{\text{cov}}$).
...
Para facilitar la experimentación, inspección visual y demostración en tiempo real de los algoritmos desarrollados en este trabajo, se ha implementado un conjunto de cuadernos interactivos basados en widgets de Jupyter (\texttt{Notebook\_Demo\_Rapida\_Interactiva.ipynb}), cuya interfaz completa se presenta en la Figura~\ref{fig:demo_interactiva}.

\begin{figure}[H]
	\ffigbox[\textwidth]
	{\caption{INTERFAZ GRÁFICA DEL CUADERNO INTERACTIVO DE SIMULACIÓN Y CONTROL: PANEL DE CONFIGURACIÓN DE PERFILES LPB (SUPERIOR), SELECTOR DE ALGORITMOS MTS/SARENV Y PRESUPUESTO DE VUELO (CENTRO), Y VISTA DUAL DE TRAYECTORIA, MAPA RESIDUAL Y CUADRO DE MÉTRICAS SAR (INFERIOR)}}
	{\label{fig:demo_interactiva}
	\includegraphics[width=0.98\textwidth]{imagenes/interfaz_demo_interactiva.png}}
\end{figure}

Esta herramienta proporciona al operador un entorno unificado de control que integra los cinco módulos del framework:
\begin{enumerate}
    \item \textbf{Configuración de la Víctima (Panel Superior):} Permite cargar perfiles rápidos basados en la clasificación LPB de Koester \cite{koester2008lost} (Autista, Demencia o Senderista) o personalizar libremente mediante deslizadores los pesos asignados a cada capa semántica de OpenStreetMap (\textit{structure}, \textit{road}, \textit{woodland}, \textit{water}, \textit{field}, \textit{brush}), así como consultar los radios estadísticos de dispersión por cuartiles.
    \item \textbf{Control de Simulación y Selección Dual de Algoritmos (Panel Central):} Permite al operador alternar en tiempo real entre las dos familias algorítmicas integradas en el framework: por un lado, los planificadores metaheurísticos bioinspirados de optimización discreta de \textbf{MTS} (\textit{Colonia de Hormigas ACO}, \textit{Colonia de Abejas ABC}, \textit{Agujero Negro BHA} y \textit{Voraz Heurístico}), y por otro, los algoritmos y patrones de cobertura continuos nativos de \textbf{SAREnv} (\textit{Espiral Continua / Expanding Spiral}, \textit{Barrido Zigzag / Lawnmower} y \textit{Voraz Continuo / Greedy}). Asimismo, permite graduar mediante deslizadores el presupuesto de pasos de vuelo de la aeronave y fijar la semilla pseudoaleatoria de reproducibilidad antes de pulsar el botón de ejecución interactiva.
    \item \textbf{Visualización y Métricas en Tiempo Real (Panel Inferior):} Presenta de forma sincronizada la trayectoria ejecutada por el UAV sobre el mapa base de calor (izquierda), la evolución dinámica del mapa de creencias residual $b(v^{\text{final}})$ aplicando la huella sensorial circular de $R_d = 50\text{ m}$ mediante el filtro recursivo bayesiano (derecha) y el cuadro de mando estandarizado de métricas SAR (\textit{SAR Evaluation Metrics Dashboard}), reportando el estado de detección, la distancia euclídea mínima a la víctima, la longitud de ruta recorrida, el área geográfica barrida neta y la masa de probabilidad absorbida.
\end{enumerate}
