"""Búsqueda voraz (Limited Depth First Search) usando la heurística miope"""
...
from busquedas import greedy_a
from busquedas.heuristicas import heur_local
...
from uiux import generar_malla
...
#Hay que pasarle el tamaño del cuadrado de la malla (1 unidad de desplazamiento = 1 cuadrado, que puede ser X metros)
...
new_list_x, new_list_y, new_list_z, steps, finder, new_bk = rh_search(
#new_list_x, new_list_y, new_list_z, steps, finder, new_bk = greedy_a.rh_search_a(
    size,
    init_pos,
    N_AGENTS,
    min_dist,
...
# Se hace la transformación de las coordenadas
...
print(df)
guardar_dataframe(
    df, "./resultados/", "ldfs-myop", file.split("/")[-1].split(".")[0]
)

"""dibujar_animacion(
    N_AGENTS, list_x, list_y, list_z, BK, goal, size, steps, seed, dmax, HEIGHT
)"""
...
#interfaz_a.plot_probability_with_trajectory(BK, size, tc, list_x, list_y, list_z)

# Generar archvo .plan
if GENERAR_PLAN:
    latitude_0, longitude_0 = reference_coordinate #mis coordenadas de inicio del dron
