...
def heur_correcion_miopia(
    ...
    # Precalcular el logaritmo para evitar recalcularlo en el bucle
    ...
    for i, cs in enumerate(next):
        ...
        # Simplificación matemática directa: 1 - sum((1 - lambda^dist) * bk) = lambda^dist
        # Esto evita realizar una suma de 220,000 elementos por cada sucesor, logrando un speedup masivo
        # y previniendo la cancelación catastrófica (underflow) cuando el objetivo está lejos.
        info[i] = np.exp(log_lambda * dist)
